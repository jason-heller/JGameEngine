...
public class Cell {
	...
	public void setType() {
		type = (int) (Noise.noise2d((long)center.x, (long)center.y,BIOME_SEED)*4f);
	}
	
	public boolean intersectsBox(float bx, float by, float bs) {
		if (contains(bx+1,by+1)) return true;
		float l = bs-1;
		if (contains(bx+l,by+l)) return true;
		if (contains(bx+1,by+l)) return true;
		if (contains(bx+l,by+1)) return true;
		
		boolean lastResult = lineInRect(bx, by, bs, edges.get(0).x1,edges.get(0).y1, edges.get(0).x2,edges.get(0).y2);
		
		int len = edges.size() - 1;
		for(int i = 1; i < len; i++) {
			if (lastResult != lineInRect(bx, by, bs, edges.get(i).x1,edges.get(i).y1, edges.get(i+1).x2,edges.get(i+1).y2)) {
                return false;
            }
		}
	...
	private boolean onSameSide(float x, float y, GraphEdge edge) {
		...
		if (
			(dy*(center.x-edge.x1)+dx*(center.y-edge.y1))
		   *(dy*(x-edge.x1)       +dx*(y-edge.y1)) < 0
				) {
			return false;
		}
