...
import proceduralgen.HMath;
import proceduralgen.InfiniteVoronoi;
...
public class Voronoi {
	...
	private HashMap<Long, List<IntersectionGraphEdge>> intersections;
	...
	private void pruneIntersections() {
		...
		for (long l : intersections.keySet()) {
			if (intersections.get(l).size() < 4) {
				removeThese.add(l);
			}
		}

		for (long l : removeThese) {
	...
	private void pushGraphEdge(Site leftSite, Site rightSite, float x1, float y1, float x2, float y2) {
		/*if (x1 == InfiniteVoronoi.minX || x2 == InfiniteVoronoi.minX || y1 == InfiniteVoronoi.minY
				|| y2 == InfiniteVoronoi.minY) {
			return;
		}
		...
		int p1x = (int) x1, p1y = (int) y1, p2x = (int) x2, p2y = (int) y2;

		Vector2f d = new Vector2f(x2 - x1, y2 - y1);
		d.normalize();
		...
		Cell c1 = cells[newEdge.site1];
		Cell c2 = cells[newEdge.site2];
	...
	private void markPts(int x, int y, float x1, float y1, GraphEdge edge) {
		int p1x = Math.floorDiv(x, scene.world.terrain.Terrain.chunkSize), p1y = Math.floorDiv(y, scene.world.terrain.Terrain.chunkSize);
		long key = HMath.szudzik(p1x, p1y);
		IntersectionGraphEdge pt = new IntersectionGraphEdge(x1, y1, edge);
	...
	public List<IntersectionGraphEdge> getIntersectionPts(int x, int y) {
		return intersections.get(HMath.szudzik(x, y));
	}
