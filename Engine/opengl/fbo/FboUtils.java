...
public class FboUtils {
	...
	public static int createDepthBufferAttachment(int width, int height, boolean multisampled) {
		...
		if (multisampled) {
			GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, Globals.fboSamplingAmt, GL11.GL_DEPTH_COMPONENT, width,
					height);
		} else {
	...
	public static int createMultisampleColorAttachment(int width, int height, int attachment) {
		...
		GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, Globals.fboSamplingAmt, GL11.GL_RGBA8, width, height);
