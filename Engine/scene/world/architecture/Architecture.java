...
import scene.world.architecture.render.ArcRender;
		...
		if (Debug.ignoreBsp) {
			for(BspLeaf leaf : bsp.leaves) {
				...
				for(VisibleObject visObj : leaf.getVisibleObjects()) {
					ObjectControl.renderTriList(camera, visObj);
				}
			}
		} else {
			...
			for(BspLeaf leaf : renderedLeaves) {
				for(VisibleObject visObj : leaf.getVisibleObjects()) {
					...
					ArcRender.render(camera.getProjectionMatrix(), camera.getViewMatrix(), lightmap, visObj);
				}
			}
			...
			if (Debug.wireframe) {
				for(BspLeaf leaf : renderedLeaves) {
					...
					for(VisibleObject visObj : leaf.getVisibleObjects()) {
						...
						ObjectControl.renderTriListWireframe(camera, visObj);
