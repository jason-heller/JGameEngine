...
public class Camera {
	...
	public void move() {
		...
		if (controlStyle == NO_CONTROL && focus != null) {
			if (lookAt == null) {
				...
				if (focus instanceof NPC) {
					lookPos.y += ((NPC)focus).getHeadOffset();
				}
	...
	private void handleControl() {
		...
		if (controlStyle == SPECTATOR || controlStyle == FIRST_PERSON) {
			if (Mouse.isGrabbed()) {
				float offset = 1;
				float pitchChange = Input.getMouseDY() * (sensitivity/(offset));
				float angleChange = Input.getMouseDX() * (sensitivity/(offset));
				pitch-=pitchChange;
	...
	public void updateViewMatrix() {
		...
		viewDirection = MathUtils.eulerToVector(yaw, pitch);
