...
public class Temp {
	public static int noise(double x, double y, int nbOctave) {
		...
		while (octave != 0) {
			...
			/*
			 * NOTE : on for true grandiant noise uncomment following block for true
			 * gradiant we need to perform scalar product here, gradiant vector are
			 * created/deducted using the above pseudo random values (alt1...alt4) : by
			 * cutting thoses values in twice values to get for each a fixed x,y vector
			 * gradX1= alt1&0xFF gradY1= (alt1&0xFF00)>>8
			 *
			 * the last part of the PRN (alt1&0xFF0000)>>8 is used as an offset to correct
			 * one of the gradiant problem wich is zero on cell edge
			 *
			 ...
			 *
			 * all the rest of the code (interpolation+accumulation) is identical for value
			 * & gradiant noise
			 */
			...
			// BiCubic interpolation ( in the form alt(bX) = alt[n] - (3*bX^2 - 2*bX^3) *
			// (alt[n] - alt[n+1]) )
