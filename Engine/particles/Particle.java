...
public class Particle {
	...
	public Particle(Texture texture, Vector3f position, Vector3f velocity, float gravity, float life, float rotation, float rotationSpeed, float scale) {
		this(texture, position, velocity, gravity, life, rotation, 0, scale, 0, texture.getNumAtlasRows()*texture.getNumAtlasRows());
	}
	...
	private void setTextureOffset(Vector2f offset, int index) {
		int column = index % texture.getNumAtlasRows();
		int row = index / texture.getNumAtlasRows();
		offset.x = (float)column / texture.getNumAtlasRows();
		offset.y = (float)row / texture.getNumAtlasRows();
	}
