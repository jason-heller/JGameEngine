...
public class TextureUtils {
	...
	public static Texture createTexture(byte[] rgba, int width, int height) {
		...
		TextureData textureData = new TextureData(buf, width, height);
		textureData.type = GL11.GL_TEXTURE_2D;
		int textureId = loadTextureToOpenGL(textureData);
		return new Texture(textureId, textureData.getWidth(), true, 0);
	}
	...
	public static Texture createTexture(byte[][] rgba, int width, int height) {
		...
		for(int i = 0; i < 6; i++) {
			...
			buf.flip();
			GL11.glTexImage2D(GL13.GL_TEXTURE_CUBE_MAP_POSITIVE_X+i, 0, GL11.GL_RGBA, width, height, 0, GL12.GL_BGRA, GL11.GL_UNSIGNED_BYTE, buf);
			buf.clear();
		}
