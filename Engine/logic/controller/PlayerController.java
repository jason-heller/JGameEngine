...
public class PlayerController {
	...
	public static float jumpVelocity = 35;
	public static float friction = 4f, airFriction = 0f;
	public static float maxSpeed = 65f;
	public static float accelSpeed = 250f, waterAccel = 70f;
	
	private static float cameraHeight = 7f;
	private static Vector3f cameraPos = new Vector3f();
	private static float height = 8f;
	private static float width = 2f;
	private static short health = 0;
	...
	public static void update() {
		...
		if (controllable) {
			...
			if (entity.isSubmerged()) {
				...
				if ((entity.isGrounded() || (entity.isSubmerged() && entity.velocity.y < 0)) && JUMP) {
	...
	private static void weaponControl() {
		...
		if (WeaponControl.getWeapon().isAuto()) {
			if (Input.isMouseDown(0) && Mouse.isGrabbed()) {
				WeaponControl.discharge(scene);
			}
		} else {
			if (Input.isMousePressed(0) && Mouse.isGrabbed()) {
				WeaponControl.discharge(scene);
			}
		}
		
		if (Input.isPressed("reload") && Mouse.isGrabbed()) {
			WeaponControl.reload(scene);
		}
	}
	...
	public static void spawn() {
		...
		if (spawn == World.DEFAULT_SPAWN_POINT) {
			scene.getCamera().grabMouse();
			scene.getCamera().setControlStyle(Camera.SPECTATOR);
			Console.mouseWasGrabbed = true;
		}
		...
		scene.getCamera().setYaw(90+spawn.rotation.y);
	...
	public static void postUpdate() {
		...
		if (cam.getControlStyle() == Camera.FIRST_PERSON) {
			cam.getPosition().set(cameraPos);
			Vector3f target = new Vector3f(entity.position.x,entity.position.y+cameraHeight ,entity.position.z);
			cameraPos.set(target);
			// TODO: Camera lerp?
	...
	public static void setHealth(short health) {
		PlayerController.health = health;
		
		if (health == 0) {
			scene.getGui().getSource().play("explosion");
		}
	}
