...
public class Polygon {
	...
	public Polygon(Vector3f p1, Vector3f p2, Vector3f p3) {
		...
		min = new Vector3f( Math.min(p3.x, Math.min(p1.x, p2.x)),
				Math.min(p3.y, Math.min(p1.y, p2.y)),
				Math.min(p3.z, Math.min(p1.z, p2.z)));
		max = new Vector3f( Math.max(p3.x, Math.max(p1.x, p2.x)),
				Math.max(p3.y, Math.max(p1.y, p2.y)),
	...
	public Polygon(float p1_x, float p1_y, float p1_z, float p2_x, float p2_y, float p2_z, float p3_x, float p3_y, float p3_z) {
		this(	new Vector3f(p1_x,p1_y,p1_z),
				new Vector3f(p2_x,p2_y,p2_z),
				new Vector3f(p3_x,p3_y,p3_z));
	}
	...
	public boolean containsPoint(Vector3f point) {
		Vector3f v0 = Vector3f.sub(p1, point);
		Vector3f v1 = Vector3f.sub(p2, point);
		Vector3f v2 = Vector3f.sub(p3, point);
	...
	public float barryCentric(float relx, float rely) {
		...
		float l1 = ((p2.z - p3.z) * (relx - p3.x) + (p3.x - p2.x) * (rely - p3.z)) / det;
		float l2 = ((p3.z - p1.z) * (relx - p3.x) + (p1.x - p3.x) * (rely - p3.z)) / det;
		float l3 = 1.0f - l1 - l2;
	...
	public static Polygon transform(Polygon polyNotTransformed, Matrix4f matrix) {
		Polygon poly = new Polygon(polyNotTransformed);
	...
	public void transform(Matrix4f transform) {
		Vector4f t = new Vector4f(p1.x, p1.y, p1.z, 1f);
		t.mul(transform);
		p1.set(t.x,t.y,t.z);
	...
	public void draw() {
		LineRenderer.render(Application.scene.getCamera(), p1, p2);
		LineRenderer.render(Application.scene.getCamera(), p1, p3);
		LineRenderer.render(Application.scene.getCamera(), p3, p2);
	}
}
